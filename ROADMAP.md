...
**Shipped status (2026-06-12):**

| PR | Status | Tag / commit |
|---|---|---|
| PR-1 schema + minimal CLI | ✅ Shipped | v2.15.6 |
| PR-1.5 API-provider write/edit/exec tools (sandbox + bash allowlist) | ✅ Shipped | v2.16.0-alpha.1 |
| PR-2 `ato missions dispatch` (budget gates, state transitions, audit events) | ✅ Shipped | v2.16.0-alpha.2 |
...
**Will 2026-06-12:** Google AI Studio shows R$0,58 (~$0.107) month-to-date for the Gemini key; ATO's Settings cost panel shows $0.014 for the same key — **~7× undercount**. If the cash amounts are wrong, model-vs-model cost comparisons (the v2.10 wedge) are wrong.

**Scope is ALL 8 registry providers, two tiers (Will, explicit):** keyed providers (anthropic / openai / google / minimax) get code audit + billing-portal reconciliation; unkeyed registry providers (grok / deepseek / qwen / openrouter) get a code-level audit — correct published rates, correct per-flavor token-class extraction, reasoning-token handling (deepseek-reasoner, grok thinking), openrouter passthrough pricing handled or honestly marked unsupported — so costs are right for the users who do hold those keys. All providers: cost math must include every billed token class (google `thoughtsTokenCount` + `cachedContentTokenCount`; anthropic cache-creation/cache-read tiers; openai reasoning tokens + cached-input discounts; minimax quirks). Suspects: (1) thinking/reasoning tokens excluded from the **cost** math (the v2.7.15 fix covered token *counts*; gemini-3 thinking is billed at output rates and dominates thinking-heavy calls); (2) stale rates in the pricing registry (preview models first); (3) error dispatches (503s) unbilled by ATO but partially billed by providers. Deliverables: per-provider cost-math audit vs published rates, month-to-date reconciliation of `execution_logs` against every billing portal, pricing-registry fix, real-data regression test (n≥30 receipts vs known billing), and an honest accuracy-class split in the Settings panel (exact BYOK rows vs ±15% heuristic subscription/pre-migration rows). Queued after PR-4.
