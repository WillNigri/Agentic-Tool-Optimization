...
- **Per-dispatch agentic-mode toggle** (need-to-have). Right now codex dispatch is unconditionally `workspace-write`. Add a `--read-only` flag to `ato dispatch` and a UI toggle in the war-room launcher for pure review/feedback war-rooms where you don't want any seat editing files. Default stays agentic (the current behavior); opt-in to read-only. Without this we force every war-room to be agentic, which is wrong for "compare these models' opinions on this PR" scenarios. Touches the dispatch CLI surface + `WarRoomLauncher` UI.
