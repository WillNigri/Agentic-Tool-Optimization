...
// ── v2.3.20 Phase 5.5 — Activity feed (posts) Tauri commands ──────────
//
// Thin wrappers around crate::posts::* and the approval-flow CLI's
// logic, exposed to the React frontend. Heavy validation lives in
// the ato-posts shared crate; these commands just thread DB
// connections and translate errors.

#[tauri::command]
pub fn posts_list(
    limit: usize,
    kind: Option<String>,
) -> Result<Vec<crate::posts::Post>, String> {
    let conn = rusqlite::Connection::open(crate::get_db_path()).map_err(|e| e.to_string())?;
    let kind_parsed = match kind.as_deref() {
        Some("message") => Some(crate::posts::PostKind::Message),
        Some("event_notice") => Some(crate::posts::PostKind::EventNotice),
        Some("approval_request") => Some(crate::posts::PostKind::ApprovalRequest),
        Some("approval_decision") => Some(crate::posts::PostKind::ApprovalDecision),
...
pub fn posts_create(
    ...
    kind: Option<String>,
) -> Result<crate::posts::Post, String> {
    let conn = rusqlite::Connection::open(crate::get_db_path()).map_err(|e| e.to_string())?;
    let ak = match author_kind.as_str() {
        "human" => crate::posts::PostAuthorKind::Human,
        "agent" => crate::posts::PostAuthorKind::Agent,
        "system" => crate::posts::PostAuthorKind::System,
        other => return Err(format!("unknown author_kind '{}'", other)),
    };
    let k = match kind.as_deref().unwrap_or("message") {
        "message" => crate::posts::PostKind::Message,
        "event_notice" => crate::posts::PostKind::EventNotice,
        "approval_request" => crate::posts::PostKind::ApprovalRequest,
        "approval_decision" => crate::posts::PostKind::ApprovalDecision,
...
#[tauri::command]
pub fn posts_pending(limit: usize) -> Result<Vec<crate::posts::Post>, String> {
    let conn = rusqlite::Connection::open(crate::get_db_path()).map_err(|e| e.to_string())?;
...
pub fn posts_decide(
    ...
    Ok(crate::posts::Post {
        ...
        author_slug: None,
        kind: crate::posts::PostKind::ApprovalDecision,
        text: body,
        related_event_seq: None,
...
// decide path. Add the re-export at the storage module so this
// command can call it without bouncing through ato_posts directly.
