...
// Owns: every recipe-action executor + the dispatch fn that
// routes RecipeAction variants to its handler. Reader helpers
// (read_capped/drain_reader/abort_reader/truncate_chars) live
// here because they're scoped to run_script + post_webhook;
// not template helpers (those live in placeholders.rs).
//
// Sibling visibility: execute_action + ActionOutcome are
// pub(super) so mod.rs's handle_event can dispatch.

use crate::events::AtoEvent;
use std::time::Duration;
use crate::recipes::RecipeAction;

use super::placeholders::{EventFields, extract_event_fields, first_missing_placeholder, redact_url, apply_substitution, substitute_simple_placeholders};

// ─── Action executors ──────────────────────────────────────────────────

pub(super) struct ActionOutcome {
    pub(super) status: &'static str, // "done" | "failed" | "not_implemented"
