...
- [x] v2.7.3 release — tag `b01a6a4`; CI artifacts built; Homebrew tap updated (`c63dfd0` corrected the auto-bump sha256 mismatch); `brew install --cask willnigri/ato/ato` smoke verified ato 2.7.3 binary on /Applications/ATO.app.
- [x] PR 8 — rename `ephemeral` → `single_run` (`dc65a3e`). Kept the existing "Single runs" filter terminology; `single_run` singular for individual rows, `single_runs` plural for the filter chip. Codex Round 1 caught 2 grammar bugs from the bulk substitution; fixed before push.
...
- [x] PR 11 — `project_id` snapshot at session create (`4982d4e`). `--project` flag on `ato sessions new`; `useProjectStore.activeProject` snapshot in `NewSessionModal`; CLI validator with the three-state model (no projects table → silent None; table-present-id-missing → stderr warn + None; valid → use). 5 new unit tests; 13 total green.
