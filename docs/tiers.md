...
| **Team-shared conversations** — `ato war-rooms share / unshare / list-shared`, `ato sessions share`, `ato chats share` (v2.18.7) | **Team** | Share any war-room, session, or chat into a team workspace. The item appears in every teammate's Sessions feed as a rich card (title, summary, tags, runtime badges, 👥 TEAM badge, "shared by <name>", member avatars) — identical fidelity to a local card. The owner sees one card (no duplicate). Codified share-from-CLI so agents can share programmatically after closing a war-room. Free DIY: export the war-room transcript by hand and paste it into a shared doc. |
| **Real-time team append** — `ato war-rooms append-event <id> --team <slug> --kind <kind> --json <payload>` (v2.18.7) | **Team** | Append a turn to a shared war-room/session/chat and every connected member sees it live (HTTP append → Postgres NOTIFY → WebSocket push). Both directions, no refresh. Previously team shares were read-only snapshots. Free DIY: not replicable without the cloud relay. |
...
| Close coordinator summary (`ato war-rooms close --coordinator claude\|codex\|gemini`) | Free | Subscription-based close: prefers the active Claude Code / Codex / Gemini CLI subscription (no API key billing). Falls back to a stored API key if the subscription is unusable. Desktop close dialog exposes a Subscription vs API picker. |
