...
- **Replace the desktop's Upgrade button** with `curl -X POST https://api.agentictool.ai/api/billing/checkout -H "Authorization: Bearer <jwt>" -H "Content-Type: application/json" -d '{"tier":"pro","successUrl":"ato://billing/success","cancelUrl":"ato://billing/cancel"}'` and open the returned `data.url` in your browser. The desktop's `lib/billing.ts#startCheckout` codifies the JWT lookup, the redirect deep-links, and the 402 PRO_REQUIRED fallback to the founder-led onboarding call. The endpoint itself is in OSS-accessible cloud infra — the button is the convenience.
