...
### 2. Summon specialists by name

The roster is pre-loaded. As CEO, you summon the specialists whose
domain the decision touches. Two or more per war-room, matched to the
decision class. Each summoned specialist gets the Karpathy-filtered
prompt from step 3.

| Decision class                       | gstack specialist(s) — invoke via `Skill(<name>)`      | ATO dispatch (cross-family)         |
|--------------------------------------|--------------------------------------------------------|-------------------------------------|
| Plan / multi-stage scope             | `autoplan` (CEO → design → eng → DX, automated)        | MiniMax or Gemini outside view      |
| Strategy / positioning / pricing     | `office-hours` + `plan-ceo-review`                     | MiniMax founder-mode                |
| Architecture / data flow / tests     | `plan-eng-review`                                      | MiniMax or DeepSeek senior eng      |
| UI / UX / IA / visual                | `plan-design-review` + `design-consultation`           | Gemini for design taste             |
| Developer-facing surface (CLI/API)   | `plan-devex-review`                                    | MiniMax DX-mode                     |
| Encryption / auth / IPC / threat     | `cso` (OWASP + STRIDE)                                 | MiniMax or DeepSeek security        |
| Performance / scaling / cost         | `plan-eng-review` (perf section)                       | MiniMax systems-engineer            |
| Debugging / "why doesn't this work"  | `investigate`                                          | MiniMax or Gemini adversarial       |
| Cut-or-ship / "is this the wedge"    | `office-hours` + `plan-ceo-review` (Reduction mode)    | MiniMax founder-mode                |
| Need an adversarial second opinion   | `codex` (adversarial-challenge mode)                   | `codex` doubles as the cross-family leg |

These specialists are "ready to be summoned" — they're gstack skills
installed at `~/.claude/skills/` or vendored under
`<repo>/.claude/skills/`. The CEO calls them by name. They are NOT
auto-fired; summoning is a deliberate CEO action.

Default if the decision class isn't on the table: summon
`plan-ceo-review` as self-challenge + `ato dispatch minimax` as
cross-family. Adjust upward (more specialists) for higher-stakes
decisions.

You remain in the CEO seat regardless of who else is summoned. The
specialists advise; the CEO decides.
